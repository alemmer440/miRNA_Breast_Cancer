...
#Create Volcano Plot
p <- ggplot(data=resultdf, aes(x=log2FoldChange, y=-log10(padj), col=diffexpressed)) +
  geom_point(size=4) +  scale_color_manual(values = c("Not Significant" = "black",  "Significant"="red")) +
  ggtitle("Differential miRNA expression in breast cancer \ntumors compared with healthy tissue")+
  theme(plot.title = element_text(hjust = 0.5))+ ylab("-log10(pvalue)")+
  theme(legend.title = element_blank())+ theme(text = element_text(size = 35))
